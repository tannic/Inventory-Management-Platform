				...
				$this.bootstrapWizard({
					tabClass: "",
			  		...
			  		onNext: function($tab, $navigation, index){
						...
						if(index==2){
							$('#employees').dataTable().fnAddData(["Administrator", $("input#username").val(), "" ]);
							if(!usersub){
								$.ajax({
								async:false,
								method: 'POST',
							...
							if(!companysub){
    							...
    							dataType: "json",
								data:JSON.stringify({
									name:$("input#company_name").val(),
									email:$("input#company_email").val(),
									phone:$("input#company_phone").val(),
									registration_no:$("input#registration_no").val(),
									sales_channel:'/api/v1/saleschannel/8/',
									default_currency:'/api/v1/currency/1/',
									address:'',
								}),
								success: function(response){
									companyid=response.id;
									company_resource=response.resource_uri;
								},
								...
								$.ajax({
										...
										async:false,
										type:'get',
										url:'/api/v1/employeerole/',
										success: function(response){
											var html;
											response.objects.forEach(
											function(entry){
												if(!rolehtml){
													rolehtml="<option value="+entry.id+">"+entry.description+"</option>";
												}
												else{
													rolehtml=rolehtml+"<option value="+entry.id+">"+entry.description+"</option>";
												}
											});
										},
										error: function(response){
    							...
    							dataType: "json",
								data:JSON.stringify({
									first_line:$("input#firstline").val(),
									second_line:$("input#secondline").val(),
									city:$("input#company_city").val(),
									zipcode:$("input#company_postcode").val(),
									country:'/api/v1/country/'+$('#countrylist').find(":selected").val()+'/',
									is_default:'true',
								...
								error: function(response){
									console.log('/api/v1/country/'+$('#country_list').find(":selected").val()+'/');
									rs=false;
								},
								});
							}
								...
								async:false,
								method:'PUT',
								url:'/api/v1/addcompany/'+companyid+"/",
								contentType: "application/json; charset=utf-8",
    							dataType: "json",
								data:JSON.stringify({
									name:$("input#company_name").val(),
